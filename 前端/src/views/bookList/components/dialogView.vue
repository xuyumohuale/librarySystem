...
const emit = defineEmits(["closeDialog", "success_reserve"]);
...
const reserveBody = reactive({
  userid: userid(),
  bookid: bookData.value.id
});
watch(
  () => useUserStoreHook()?.id, // 监听userid的变化
  val => (reserveBody.userid = val)
);
watch(
  () => bookData.value.id, // 监听bookid的变化
  val => (reserveBody.bookid = val)
);
const sure = async () => {
  reserveBook(reserveBody).then(res => {
    if (res.success) {
      setTimeout(() => {
        message(res.message, {
          type: "success"
